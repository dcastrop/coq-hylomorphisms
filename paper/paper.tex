...
a map between functors is a family of morphisms $\phi_{X} : FX \to GX$ indexed
by the objects $X \in \Obj(\CatC)$ and such that it is \emph{natural} in $X$,
that is $ G(f) \cdot \phi_{X} = \phi_{Y} \cdot F(f)$. Intuitively, a natural
transformation transforms the structure of a functor into the structure of
another functor without assuming what is the type of data contained in them, for
example, a program $f : \texttt{List }X \to \texttt{Maybe} X$ which returns
nothing if the list is empty and the head of the list otherwise needs not to
...
$P(s) \to X$. Furthermore, extraction will need to insert unsafe casts for
...
denotes setoid equality. All morphisms need to be \emph{proper} morphisms that
respect setoid equalities. We use special notation for such morphisms, \coq{A ~>
  B}, and we add an implicit coercion from \coq{A ~> B} to \coq{A -> B}.  We
also provide tactics to automatically discharge proofs that morphisms are
proper,  for some simple cases.
...
We define containers in terms of shapes, \emph{base} positions (\coq{APos :
  Set}), and \emph{decidable validity predicates} that specify when a base
position is valid in a shape. Since validity predicates are in \coq{Prop}, they
will be erased during extraction, and since they are decidable, they do not
...
\coq{times_two} multiplies every element of the list by two. We can use Coq's
generalised rewriting and \coq{hylo_fusion_l} to fuse \coq{times_two} into
...
We show now show the formalisation and extraction of \emph{dynamorphisms} for
dynamic programming using the knapsack example from~\cite{HinzeWG15}. In
general, dynamorphisms build a memoisation table which stores intermediate
results alongside the current computation. The algebra used to define a
dynamorphism can access this memoisation table to speed up computation.
First, we define memoisation tables in terms of containers:
\begin{coqcode}
Instance Memo : Cont (A * Sg) Pg := { valid := valid \o pair (snd \o fst) snd }.
Definition Table := LFix Memo.
\end{coqcode}
Given a type of shapes \coq{Sg} and positions \coq{Pg}, the container
\coq{Memo : Cont (A * Sg) Pg}.  The least fixed-point of which is the least
fixed-point of the

given a container \coq{G : Cont Sg Pg}
