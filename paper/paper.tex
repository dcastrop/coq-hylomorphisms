...
For example, for a type of states \coq{X} and an alphabet type \coq{L} we can
define a (labelled) transition system (LTS) on \coq{X} as a
function \coq{c : X -> L * X} implementing the \emph{transition} map. In
particular, for a state \coq{x1 : X}, \coq{c x1} returns a
pair \coq{(l, x2) : L * X} where \coq{l : L} is the observable action
and \coq{x2 : X} is the next state. We could then add the following notation
\mpav{is this notation duable in Cock?}
\[
  x_{1} \xrightarrow{l} x_{2} \eqiff c x_{1} = (l, x_{2})
\]
\begin{coqcode}
Notation x1 --l> x2 := c(x1) = x2.
\end{coqcode}
...
We define smart constructors for the isomorphism of greatest fixed points
\begin{coqcode}
