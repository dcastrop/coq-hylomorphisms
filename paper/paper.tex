...
$a(\operator{x,y}) = (\neg x, \neg y)$ and hence an hylomorphism as defined in (\ref{eq:cata}).  Now we are left to show they are
equivalent. Well, it is easy to do so since
$\operator{even} = \pi_{1} \comp \operator{even-odd}$
...
present work. In this paper, we will instead derive the recursion schemes as needed and
implement them as hylos.
...
they are the unique solution to the equation \mintinline{Coq}{f =e g \o fmap f
\o l_out}). The proof is by induction on the input to \mintinline{Coq}{f} and
