...
\end{minted}
Note the use of \mintinline{Coq}{=e} instead of Coq's standard equality.  For
...
know that \mintinline{Coq}{App F X} is isomorphic to \mintinline{Coq}{A}, and
\mintinline{Coq}{App G X} is isomorphic to \mintinline{Coq}{B}. Then we can
show that \mintinline{Coq}{App (Prod F G) X} is isomorphic to
\mintinline{Coq}{A * B}.  If we have an element of type 
\mintinline{Coq}{App (Prod F G) X},
using the \mintinline{Coq}{inl} position, we can obtain
\mintinline{Coq}{App F X}.  Similarly, using \mintinline{Coq}{inr}, we can
obtain \mintinline{Coq}{App G X}. Since these are the only two valid positions
in the shape of pairs, we have finished. It is now sufficient to use the
isomorphisms of \mintinline{Coq}{App F X} and \mintinline{Coq}{App G X} to
obtain \mintinline{Coq}{A * B}. Similarly if we have \mintinline{Coq}{A * B},
we can first use the isomorphisms of 
\mintinline{Coq}{A} and
\mintinline{Coq}{B} to obtain
\mintinline{Coq}{App F X * App G X}, and then construct the necessary container
extension. Given \mintinline{Coq}{p_inl}/\mintinline{Coq}{p_inr} that act as
\mintinline{Coq}{inl/inr} but on valid product positions, the relevant
...
\end{minted}
Catamorphisms from this initial algebra are denoted by \mintinline{Coq}{ccata},
and they can be used to define the isomorphism between 
\mintinline{Coq}{{x : GFix F | FinF x}} and
\mintinline{Coq}{LFix F}. In particular: 
\begin{minted}{Coq}
...
is that \mintinline{Coq}{h} can only be applied finitely many times:
\begin{minted}{Coq}
...
they can only be applied finitely many times (\mintinline{Coq}{RecF}).  Note
...
\end{minted}
Here, \mintinline{Coq}{RecF_inv} provides the structurally smaller element for
...
\mintinline{Coq}{f =e l_in \o fmap f \o h},
where \mintinline{Coq}{h} is a recursive coalgebra.
\begin{minted}{Coq}
...
\mintinline{Coq}{RecF_inv} to obtain the structurally smaller proof to use in
the recursive calls.  As we did with catamorphisms and recursive anamorphisms,
we prove that \mintinline{Coq}{hylo_def} is respectful, and use this proof to
