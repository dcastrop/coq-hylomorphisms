...
parallelisations~\cite{TakanoM95,Gibbons96:Third, farmsCastro}).  These
algebraic properties led to their use in the context of \emph{program
calculation}, where programmers would describe their code using simple,
inefficient specifications in an \emph{algebra of programming}~\cite{}, and then
use the algebraic laws of this algebra of programming to \emph{calculate} an
efficient version of the same algorithm.

\dcas{TODO: Introduce example}

Despite the rigorous algebraic laws that are satisfied by recursion schemes,
there is a lack of tool support for their use in the context of program
...
Haskell)~\cite{TODO:INeedToFindSomeMoreReferences}. Most of the work on
automating program calculation techniques either build implementations of the
algebraic laws, together with some form of syntax or
EDSL~\cite{TODO:INeedToFindSomeMoreReferences}. Such implementations are not
verified, and can introduce bugs in the way they apply the algebraic laws as
rewriting rules.

A more rigorous approach to implementing program calculation techniques is to
\emph{mechanise} them as part of a proof assistant. However, most of the work in
mechanising recursion schemes focuses on a narrow subset of the known recursion
schemes due to termination
issues~\cite{TODO:FillInReferencesForCoqMechanisationsFromRelatedWorkSection}.
Indeed, most of the mechanisations focus on \emph{maps} and \emph{folds}, and
few authors focus on the \emph{unfolds}. In general, Hinze et
al.~\cite{HinzeWG15} has proven that all known  recursion schemes are instances
of a  divide-and-conquer algorithm which goes under the name of
\emph{hylomorphism}~\cite{MeijerFP91,HuIT96}.  To date, no work has mechanised
hylomorphisms in the Coq proof assistant.
...
%\emph{well-founded recursion}, where recursive definitions are accompanied by
...
%infinitely decreasing chains.}.
%A benefit of this strategy is that when extracting verified non-structural
%recursive functions to OCaml, the additional proofs are discarded, resulting in
%code that closely resembles what a programmer might have written directly in
%OCaml. However, reasoning about program equivalences needs  such termination
%proofs to be kept around, leading to the common practice of employing additional
%reduction lemmas extensively.
%
%% A benefit of this approach is that, extracting
%% verified nonstructural recursive functions to OCaml will erase the additional
%% proofs, thus producing code that will be closer to what a programmer may have
...
%\emph{Structured recursion schemes}~\cite{HinzeW16, HinzeWG15} is a technique
%for structuring recursive definitions that has found a lot of success in
%functional programming. The idea is that, rather than writing a self-refential
%definition, we use a function library, the recursion scheme, which takes as
%input the non-recursive parts of the algorithm and returns the desired recursive
%definition which is guaranteed to terminate by the properties of the recursion
%scheme itself. More complex recursion schemes do exist but it has been
%proven by Hinze et al.~\cite{HinzeWG15} that they are all instances of a divide-and-conquer
%algorithm which goes under the name of hylomorphism~\cite{MeijerFP91,HuIT96}.
%This result, thus, yields a general framework to reason about the algebraic properties of these
%algorithms, e.g. short-cut fusion and semi-automatic parallelisations~\cite{TakanoM95,Gibbons96:Third, farmsCastro}.

% A more complex recursion scheme is the hylomorphism~\cite{MeijerFP91, HuIT96} which has a divide-and-conquer
% structure. It has been shown by Hinze et
% al.~\cite{HinzeWG15} that any complex recursion scheme can be transmogrified
% into an hylomorphisms by means of an adjunction. Furthermore, hylomorphism laws
% can capture a number of useful equivalences, ranging from common optimisations
% such as \emph{short-cut fusion}~\cite{TakanoM95}, to semi-automatic
...
%(e.g.) Haskell and ML, recursion schemes cannot ensure termination. This is
%because the input information itself could be non-terminating. On the other
%hand, while formalising recursion schemes in a type theory does provide stronger
%termination guarantees, to the best of our knowledge, not many attempts have
%been made at mechanising them.
...
al.'s approach, extraction (1) do not preserve the recursive structure of common
implementations; and (2) lead to unsafe casts like \ocaml{Obj.magic} in the
generated code. This latter is also problematic in that, for higher-order
...
Github\footnote{https://github.com/dcastrop/coq-hylomorphisms}. While
programmers still need to reason about the termination of their programs, our
mechanisation will allow the use of the algebraic laws of hylomorphisms for
\emph{program calculation}, as well as extracting to idiomatic code (see
extracted \ocaml{qsort} program in Section~\ref{sec:examples}).
...
\end{coqcode}
witnessing the idea that a functor represents a container for abstract data that
can be manipulated without changing the outer structure. For example, the type
\coq{List : Type -> Type} is a functor and \coq{List A} is the type of lists
over elements of type \coq{A} with the obvious \coq{fmap} function recursively
traversing a list and applying the function \coq{A -> B} for each element.
Additionally, the type of lists \coq{List A} arises as the least fixed-point of
the functor \coq{F X = unit + (A * X) }.
...
\label{sec:algebras} An algebra is a set \coq{A} (the \emph{carrier} of the
algebra) together with some operations on it sometimes subject to some
equational laws. For example, given such a type \coq{A} we can define the monoid
operations as \coq{u : unit -> A} for unit and \coq{m : A * A -> A} for
multiplication. Notice that \coq{F X = unit + X * X} is a functor on \coq{X}
which means we can equivalently describe these two maps as a single one of type
\coq{F X -> X} which we call the algebra for the functor \coq{F} satisfying the
unit and associative laws of the monoid. In this work we will be using algebras
to describe the operations of a data type and so we will not be needing
additional equations on these operations.
