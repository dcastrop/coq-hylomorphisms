...
Recursion schemes provide an abstract way to consume and generate data.  The
most simple and well-known recursion scheme is the \emph{fold} or
\emph{catamorphism} which consumes data. Its dual is the \emph{unfold} or
\emph{anamorphisms} which produces data.

For example, assuming the type of lists over a type \coq{A},
namely \coq{List A}, and given an
algebra \coq{alg : unit + A * List A -> List A} a
catamorphism \coq{cata alg : List A -> B} destructs a list into its head and
tail using a destructor \coq{InOp : List A -> unit + A * List A}. This latter is
actually an isomorphim with its inverse \coq{In : unit + A * List A -> List A}
defined by the maps \coq{Empty : unit -> List A}
and \coq{Cons : A * List A -> List A}.

After destructing the input the catamorphism is applying itsefl recursively.
This is done using functoriality of the shape of the data, which is a map of
type \coq{unit + A * List A -> unit + A * List B}. Once the recursion as
returned it result it applies the algebra \coq{alg} which composes together the
results of the recursion yielding the result of type \coq{B}.

More abstractly, assuming a base functor \coq{F} a catamorphism is the unique solution to the equation
\coq{cata alg = alg . fmap (cata alg) . InOp}.
...
destructing map \coq{InOp} is generalised to a \emph{recursive coalgebra}, that
is intuitively a coalgebra that can be applied only a finite amount of times.
In fact, in the case of \coq{divide} this is a recursive coalgebra because the
size of the list is getting smaller.
...
With this in mind we know proceed onto describing how to formalise hylomorphisms
in Coq.

\subsection{Algebras}
Algebras on container extensions are defined as expected. The carrier of the
algebra is \coq{A}, and the operations of the algebra are morphisms of the
following type:
\begin{coqcode}
