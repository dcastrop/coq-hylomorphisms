...
operations as \coq{Empty : unit -> X} and \coq{Concat : X * X -> X}. Notice
that \coq{unit + X * X} is a functor on \coq{X} which means we can equivalently
describe this algebra as a map \coq{unit + X * X -> X} (together with the
...
Now, the \emph{easiest} way to turn any type \coq{A} into a monoid is to define
it as the least fixed-point of the functor \coq{unit + A + X * X} on \coq{X}.
This is isomorphic to the type of lists over \coq{A} hence it is the least
fixed-point for the functor \coq{unit + A * X} as well. This is the least
``set'' generated by the constructors \coq{Empty : unit -> List A}
and \coq{Cons : A * List A -> List A}. These constructors give an
algebra \coq{LFix_in : unit + A * List A -> List A} which has and inverse which
we  call \coq{LFix_out}.
...
that there is a unique morphism of \coq{F}-algebras from the least fixed-point of \coq{F}:
\begin{coqcode}
