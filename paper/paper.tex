...
not allow the extraction of idiomatic OCaml code.  By this we refer to code that
(1) does not preserve the recursive structure of common implementations; and (2)
lead to unsafe casts like \ocaml{Obj.magic} in the generated code.  This latter is problematic in that, for higher-order programs, simple interoperations can lead to incorrect behaviour or
even segfaults~\cite{forster:hal-04329663} and, moreover, it invalidates
the fast-and-loose principle~\cite{DanielssonHJG06}. % The absence
% of \ocaml{Obj.magic} provides even stronger
...
Finally, in this paper we provide a framework for \emph{generic programming} with recursion
schemes in Coq, with a proof of their algebraic laws and program equivalences for clean code extraction:
\begin{itemize}
  \item In Section~\ref{sec:containers} we formalise the type of container functors suitably modified for program extraction
  \item In Section~\ref{sec:recursion-schemes} we formalise hylomorhisms and prove their universal property
  \item In Section~\ref{sec:examples} we provide examples of divide and conquer, dynamic programming, and mutual recursion algorithms and we verify the short-cut fusion optimisation and show the extracted optimised code to OCaml.
\end{itemize}
