...
\section{Mechanising Extractable Containers}

\dcas{Subsections are tentative topics to remember what I need to fill in here,
but these are not final}

Our goals are: (1) the mechanisation of hylomorphisms and their laws should be
axiom-free; and (2) Coq hylomorphisms should be extracted to recursive OCaml
functions that use no instances of \mintinline{OCaml}{Obj.magic}. Note that
although the use of \mintinline{OCaml}{Obj.magic} is safe in the code extracted
by Coq, it increases the Trusted Code Base, makes code less legible for
programmers, and its presence will likely inhibit optimisations by the OCaml
compiler\footnote{\dcas{I read this somewhere, search \& add reference/link?}}.

\subsection{Functors and Containers}

Mechanising functors in Coq is straightforward. But taking least/greatest
fixpoints of the said functors may fail, due to not being strictly positive. A
common approach to construct strictly positive functors in Coq is to use
\emph{containers}. In essence, a container 
\dcas{A first approach (problems with extraction, lots of Obj.magic, etc)}
\begin{minted}{Coq}
...
programs, including productivity termination. The core idea of sized types is
that types express bounds on the sizes of recursive data structures. This
