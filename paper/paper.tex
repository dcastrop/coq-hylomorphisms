...
proof assistant. One example of this fact is the quicksort algorithm given by
the \haskell{partition} and \haskell{quicksort} function below (written in
Haskell code):
...
The first option, helps the programmer but does not guarantee termination, since
the programmer can still feed to the recursion scheme a functor that does not
have a fixed-point or that has an algebra that does not terminate.

The second option forbids non-termination, but does not allow all terminating
programs.
...
non-termination is outright rejected.  This has been a long-standing challenge
to come up with a sound algorithm which rejects all programs that do not
terminate (soundness) and accepts as many well-defined programs as possible
