...
\begin{minted}{Coq}
  Record App (Shape : Set) (Pos : Shape -> Set) (X : Set) :=
...
SSReflect.
\begin{minted}{Coq}
Class Container (Shape : Set) (Pos : Set) := {
   valid : Shape -> Pos -> bool
}.
Coercion is_true (b : bool) : Prop := b = true.
\end{minted}
Then, we define \emph{container extensions} as
\begin{minted}{Coq}
  Record App `{container Shape Pos} (X : Set) :=
...
function \mintinline{Coq}{contents}. This means that container equality needs
...
the use of other forms of equality. In most cases, we will use either propositional equality, extensional
equality, or a mixture of both, e.g. for container extensions, we use an arbitrary equality for shapes,
extensional equality for the contained elements, but Coq's propositional equality for positions:
