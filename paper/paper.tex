...
$\cons : A \times \List{A} \to \List{A}$.  The terminal object in $\Set$ is the
\emph{unit} type, that only has one inhabitant, $* : 1$.  The $\List{-}$ type is
a functor from  $\Set \to \Set$. When a functor maps a category to itself, it is
called an \textbf{endofunctor}. The action of $\List{-}$ is to take any set $A$
to the set $\List{A}$, and its functorial action
$\List{f} : \List{A} \to \List{B}$ is given by $\List{f}(\nil(*)) = \nil(*)$ and
...
anamorphism the only possible types it can produce from the seed are
the finite ones. We call this special kind of recursion scheme \emph{recursive
...
algebra $\InIso$ for the inductive data type $\mu F$. This fact falls out
...
\coq{F : Type -> Type}
satisfying the necessary identity and composition properties, then
we will not be able to construct least/greatest fixed points of this type.  A
common solution to the strict positivity problem is the use
\emph{containers}~\cite{AbbottAG05}.  A container $S \triangleright P$ is
defined by a type of \emph{shapes} $S : \Type$ and a \emph{family of position
...
The straightforward mechanisation of containers and container extensions is
still not enough for our goals of remaining axiom-free, extracting to idiomatic
OCaml code, and avoiding \ocaml{Obj.magic} in the extracted code. The main
problem is that a container extension requires a function from a family of
positions to a type: $\forall s : S.\; P\;s \to X$. On the Coq side, dealing
with the equality of objects that contain (dependent) function types would force
us to add the functional extensionality axiom, and axiom K (or equivalent) to
deal with the heterogeneous equality of position.  For extraction, OCaml's type
system is not powerful enough to represent such dependencies in the general
case, which will lead to \ocaml{Obj.magic}.

To avoid the functional extensionality axiom, we require that all the types used
in our definitions are \emph{setoids}, where we will use \coq{x =e y} to
represent setoid equality. All morphisms need to be \emph{proper} morphisms with respect
to the function \emph{respectfulness} relation. In our development, we use
\begin{coqcode}
Notation "A ~> B" := (* proper morphisms  s.t. forall x y, x =e y -> f x =e f y *).
\end{coqcode}
We add an automatic coercion from \coq{A ~> B} to \coq{A -> B}, provide tactics
to automatically discharge proofs that \coq{forall x y, x =e y -> f x =e f y}
for the simplest cases of \coq{f}, and we provide a library with
definitions for basic building blocks: sum and product types, exponentials,
composition, identity,
\ldots

To avoid axiom K, and extract containers to idiomatic OCaml code, we
define containters in terms of shapes, \emph{base} positions (\coq{APos : Set}),
...
\coq{Prop}, they will be erased during extraction, and their decidability
will make unnecessary the use of axiom K.
\begin{coqcode}
Record App F (X : Type) := {shape : Shape F; cont : {p : APos | valid s p = true} -> X}.
\end{coqcode}

Since we require every type to be a setoid, we need to define a setoid equality
(\coq{=e}) for container functors. Given two \coq{x y : App F X}, we define
\coq{x =e y} as the setoid equality of the shapes, and the extensional equality of
the contents, given equal shapes. I.e.\ \coq{x =e y} iff
\begin{coqcode}
...
formalise algebras for container extensions (Section~\ref{sec:coq-algebras}), then
we formalise coalgebras (Section~\ref{sec:coq-coalg}) and then we put together
