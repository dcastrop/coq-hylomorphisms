...
\[
  x = \In \comp \fmap{x} \comp{c}
\]
and can be implemented by hylomorphisms
\[
  \ana{c} = \hylo{\In}{c}
\]

\subsection{Accumulators}
For an algebra $a : D(A^{P}) \to A$:
\begin{align*}
  & x : (\mu F, A) \to B\\
  & x (d, p) = a (\fmap{(\curry f)} \comp \Inop(d), p)
\end{align*}
...
\begin{align*}
  & (f,g) : (\mu F, \mu F) \to (B_{1}, B_{2})\\
  & (f,g) = (a_{1}, a_{2}) \comp (\fmap{\pair{f}{g}},\fmap{\pair{f}{g}}) \comp (\Inop, \Inop)
\end{align*}
