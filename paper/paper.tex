...
On the other hand, a key benefit is the reusability of termination proofs: once
the correctness of the divide phase is established, the same proof can be reused
for any other conquer phase we choose. Moreover, in the case of structural
...
We list the contributions of this paper. First, in Section~\ref{sec:category-theory}, we provide the theoretical background on recursion schemes and hylomorphisms. We then provide a Coq formalisation of \emph{hylomorphisms} (Section~\ref{sec:recursion-schemes}) that (1) is \emph{fully axiom-free}; (2) allows the extraction of idiomatic and efficient code; and (3) can use regular Coq equalities to do program calculation, derive correct implementations, and apply optimisations. In Section~\ref{sec:extraction}, we apply this framework to formalise practical examples of divide-and-conquer, dynamic programming, and mutual recursion algorithms. Additionally, we verify the short-cut fusion optimisation and present the extracted optimised code.
...
\emph{functor} $\Set \to \Set$, in particular, an \emph{endofunctor}, mapping objects and arrows in $\Set$ to $\Set$
itself.
...
by $\cata{\beta} : (\mu F, \InIso) \to (X, a_{X})$ and pronounced
...
isomorphism and $\OutIso : \nu B \to B\nu B$ being its inverse.

For example, the terminal coalgebra for the functor $BX = L \times X$ is the set
of infinite streams, that is the greatest solution to the domain equation
$\Str{A} \iso A \times \Str{A}$.  The uniqueness property of terminal $B$-coalgebras states that for any other $B$-coalgebra
...
``\emph{anamorphism}'' or ``\emph{unfold}''. We spell out the uniqueness property:
\begin{equation}
  \label{eq:ana:unique}
...
\]
We can model this functor by using a set $S$ defined by $A \uplus B \uplus C$ which distinguishes the three
cases, and a function $P$ which tells us the number of occurrences of $X$ in
each case: for $s = (1,a) \mapsto \emptyset$; for $s = (2, b) \mapsto 1$ the singleton set and  for $s = (3, c) \mapsto 2$ the $2$-element set.

To formalise this we need  a function $p : P \to S$ and a function
$q : P \times X \to P$.  Here, $S$ serves as a \emph{shape object} that indexes
the different cases of the functor, while $P$ represents the positions where
elements of $X$ appear. The projection $p: P \to S$ assigns each position to its
corresponding shape, and the map $q: P \to P \times X$ describes how these
positions are instantiated using elements of $X$. The functor associated with
this data is defined on objects using a dependent sum (to be precise a coend):
% \[
% F(X) = S \times_P (P \times X),
% \]
% which can also be written as a dependent sum (a coend to be precise)
\[
F(X) = \sum_{s \in S} (P(s) \to X)
\]
where  \( P(s) = p^{-1}(s) \) called the fiber of \( p \) over \( s \). Given a
function \( f: X \to Y \), the functor acts by post-composition, mapping an
element \( (s, p, x) \) in \( F(X) \) to \( (s, p, f(x)) \) in \( F(Y) \),
ensuring functoriality.
...
our definitions are \emph{setoids}, where we use \coq{x =e y} to represent
...
\end{coqcode}
The initial \coq{F}-algebra can be defined as the inductive type with a finite
number of \coq{App F}
\begin{coqcode}
...
is isomorphic its corresponding the inductive data type \coq{LFix F}.
We show this by
...
recursive recursive coalgebra \coq{c}, and the initial \coq{F}-algebra:
\begin{coqcode}
...
well to other target languages (e.g. Haskell or Lisp). In particular, we show how to formalise
 quicksort (Section~\ref{sec:sorting}), we give an example of dynamic
programming algorithm, that is knapsack (Section~\ref{sec:knap}), and show examples of the shortcut
deforestation optimisation (Section~\ref{sec:shortcut}).
...
program calculation to apply a fusion optimisation. Our accompanying code
contains both mergesort and quicksort as examples, but we will focus on
quicksort for conciseness.

The recursive structure of quicksort is given by the following
functor
\begin{coqcode}
...
\coq{i_node}.
\begin{coqcode}
Inductive Tshape L A := | Leaf | Node (ELEM : A).
Inductive Tpos := | Lbranch | Rbranch. 
\end{coqcode}
The container for \coq{ITreeF} is denoted by \coq{TreeF}. For convenience, we
define a series of wrappers \coq{a_leaf} and \coq{a_node} to construct these
containers.  Now, the container for quicksort is given by \coq{TreeF int}.
...
coalgebra \coq{qsplit} is recursive. In this specific case we have to prove that
...
the functor by some total relation. Here we give a specialised property, the
...
The resulting code extracted from \coq{hylo merge qsplit} is similar to a
...
comparable complexity\footnote{The QSort definition and correctness proof in the
equations
...
composition of these two programs is given by the following definition:
\begin{coqcode}
Definition qsort_times_two := Lmap times_two \o hylo merge qsplit.
\end{coqcode}
where \coq{Lmap times_two} is a list map function defined as a hylomorphism,
...
\coq{times_two} into \coq{hylo merge qsplit} which gives the program
\coq{hylo (merge \o natural times_two) qsplit}.
In this definition, \coq{natural} defines a natural transformation by
applying \coq{times_two} to the shapes thus multiplying every pivot by two.
...
The extracted OCaml code is a single recursive traversal:
\begin{ocamlcode}
...
\end{ocamlcode}
\todo{Write something here, don't leave it empty}{}
...
\end{coqcode}
Given the recursive coalgebra for \coq{nat}, \coq{out_nat}, knapsack is defined
as
\begin{coqcode}
...
a nonstandard approach to achieve structural recursion.  For exammple, the
...
context of compilers for functional programming lanmguages.  Fusion has been
