...
structuring recursion and provide a rigorous reasoning principle for program
...
This paper presents the first Coq formalisation of \emph{hylomorphisms} allowing for the
mechanisation of all recognised recursive algorithms. The key
contribution of this paper is that this formalisation is fully \emph{axiom-free}
allowing for the extraction of safe, idiomatic functional code. We exemplify the
...
and demonstrate that the extracted functional code for the programs formalised in our
framework is efficient, humanly readable, and runnable. Furthermore, we show the use of
...
\label{sec:intro}
Structured recursion schemes are powerful
...
\end{minted}
This program is derived by \emph{fusing} a regular \emph{quicksort} OCaml implementation, with
\ocaml{map (fun x -> x * 2)}. A lot of program calculations have this pattern:
start from a simple specification, e.g.\
$\mathsf{map}\;(\lambda x.\;x \times 2) \circ \mathsf{sort}$, and use program
equivalences and algebraic laws to rewrite it to an optimised version. We will revisit a similar example
...
subproblems,  (divide), then subsolutions are computed recursively, and,
finally, these subsolutions are put together to compute the original solution
(conquer). This general pattern of recursion induces a reasoning principle which
is called fusion law which has been exemplified in the previous paragraph.

Hylomorphism are very powerful non-structural recursive algorithms that can be
...
comes at a price. Since hylomorphisms are not necessarily structural they need
extra-care is needed when encoding these in a type theory. In particular, the
divide part of the algorithm must be \emph{recursive} that is the input needs to
be split into smaller parts (not necessarily following the structure of the
input data).
...
program calculation or code extraction (see Related
Work~\ref{sec:related-work}).
...
and eventually be able to extract humanly readable functional code which can be
run which means it cannot contain unsafe casts. These are, in particular,
problematic  for higher-order programs as simple interoperations with them can
lead to incorrect behaviour or even segmentation
faults~\cite{forster:hal-04329663} and, moreover, it invalidates the
fast-and-loose principle~\cite{DanielssonHJG06}.

To retain the hylomorphism's generality we encode data structures using
polynomial functors and to ensure our extracted code is runnable we make a point
to remain axiom-free and avoid indexed types in the definition of our recursion
schemes.
...
of the form \coq{F : Type -> Type} which are additionally \emph{strictly positive}.
...
initial \coq{F}-algebra for the container that is isomorpic to the functor
...
\coq{l r : X} in \coq{i_node}). We also note that, in this container, positions
are only valid in nodes.
\begin{coqcode}
Inductive Tshape L A := | Leaf (ELEM : L) | Node (ELEM : A).
Inductive Tpos := | Lbranch | Rbranch. 
\end{coqcode}
We name \coq{TreeF} the container that has these shapes and (base) poisitions.
...
\end{coqcode}
The container for the Quicksort hylomorphism is \coq{TreeF unit int}, with the
following algebra and coalgebra. In the below definition, we use \coq{posL} and
\coq{posR} as wrappers for \coq{Lbranch} and \coq{Rbranch} with validity proofs.
\begin{coqcode}
...
(called \coq{merge} and \coq{qsplit}), and we prove that the coalgebra
\coq{qsplit} is recursive by showing that it respects the ``less-than'' relation
on the length of the lists.
%\begin{coqcode}
%Lemma split_fin : respects_relation split (@length int) lt.  (* *) Qed.
%
%Definition tsplit : RCoalg (TreeF (list int) unit) (list int)
%  := mk_wf_coalg wf_lt split_fin.
%\end{coqcode}
The code that we extract for \coq{hylo merge qsplit} is similar to a hand-written implementation:
\begin{ocamlcode}
...
We show that the output of \coq{hylo merge qsplit} produces a sorted
permutation of the input:
\begin{coqcode}
forall (l : list int), Sorted (hylo merge qsplit l) /\ Perm l (hylo merge qsplit l).
\end{coqcode}
The proof in our framework starts by induction on the proof that \coq{qsplit}
is recursive, and that it terminates on \coq{l}, i.e. using tactic
\coq{induction (recP qsplit l)}. Then, we can unfold
\coq{hylo merge qsplit} to \coq{merge \o fmap (hylo merge qsplit) \o qsplit} using
...
\coq{times_two} into \coq{hylo merge qsplit}.
