...
\]
that is $\catafree{a} = \cata{[id_{A}; a]}$.
...
\]
where $\epsilon_{A} : D_{\infty}A \to A$ is the first projection and
...
\]
that is $\anacofree{c} = \ana{\pair{id_{A}}{c}}$.
...
First we have to assume a distributive law $\lambda : DG \to GD$.
For an algebra $a : D(D_{\infty} A) \to A$,
\begin{align*}
  & g : \mu D \to A\\
  & g = a \comp D(G_{\infty}(g) \comp \anacofree{\cata{G^{\lambda}(\InIso)}}) \comp \InOp
\end{align*}
where $G^{\lambda} (alg : D\mu F \to \mu F) = DGX \xrightarrow{\lambda} GDX \xrightarrow{G(alg)} GX  $ is a $D$-algebra on $GX$. The catamorphism of this algebra has the following type
\[
...
\begin{align*}
  ...
  & \operator{knapsack}\; c = \operator{maximum}_{0} [v + \operator{knapsack}\;wvs\; (c - w) \mid (w,v) \leftarrow wvs, w \le c]
\end{align*}

Optimisation of the knapsack with Dynamic Programming.
\begin{align*}
  & \operator{knapsack} : [(\Nat, \R)] \to \Nat \to \R\\
...
\paragraph{Histomorphism}

\begin{align*}
  & \operator{dyna} : [(\Nat, \R)] \to \Nat \to \R\\
  & \operator{knapsack}\;wvs\;c= table !! c\\
  & \qquad table  = [ks\;i \mid i \leftarrow [0..c]]\\
  & \qquad ks\; i = maximum_{0} [v + table!! (i-w) \mid (w,v) \leftarrow wvs, w \le i]
\end{align*}
