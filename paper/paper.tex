...
show that these proofs are comparable to other techniques for
...
Defined.
\end{coqcode}
We prove that the coalgebra \coq{split} is recursive by showing
...
%\end{coqcode}
The code that we extract for \coq{hylo merge split} is the following:
\begin{ocamlcode}
...
We show that the output of \coq{hylo merge split} produces a sorted
permutation of the input:
\begin{coqcode}
forall (l : list int), Sorted (hylo merge split l) /\ Perm l (hylo merge split l).
\end{coqcode}
The proof in our framework starts by induction on the proof that \coq{split}
is recursive, and that it terminates on \coq{l}, i.e. using tactic
\coq{induction (recP split l)}. Then, we can unfold
\coq{hylo merge split} to \coq{merge \o fmap (hylo merge split) \o split} using
...
extract is \coq{hylo (merge \o natural times_two) split}. In this definition,
