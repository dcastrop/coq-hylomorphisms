...
\newcommand{\dcas}[1]{\textcolor{green}{\textsc{David}: #1}}
...
\[
  h \comp \InIso = a \comp F h \qquad \OutIso \comp k = F k \comp c
\]
...
$F$-coalgebra), $\InIso$ is the isomoprhism on the least fixed-point of $F$,
namely $\mu F \xrightarrow{\iso} F\mu F$, and $\OutIso$ is the isomorphism on
the greatest fixed-point of $F$, namely $\nu F \xrightarrow{\iso} F\nu F$. When
the unique solutions to the above equations exist we denote them as $\cata{a}$
and $\ana{c}$ respectively.

However, catamorphisms (dually anamorphisms) do not capture every recursion
scheme we can think of.  As an example, consider the quicksort
algorithm:\footnote[1]{This program is written in Coq notation to be consistent,
but it is not a program that Coq can accept.}
...
\end{minted}
This program is formed of essentially two parts. The first one is the
$\operator{partition}$ program destructuring the input list in three parts, the
pivot and the other two sublists, and the second part recursing over the
sublists and composing the results back together.
...
\coq{TreeF}-coalgebra on lists over the type $A$. While we can define an \coq{TreeF}-algebra on lists over $A$
as well, that is a map \coq{alg} $:$ \coq{TreeF} $[A] \to [A]$ defined
as \coq{alg (left, x, right) = left ++ [x] ++ right}.
...
\end{minted}
We define \emph{two} initial algebras that are, as expected, unique up to
...
the contained elements generically:
the contained types may not have (yet) an associated equivalence. In other
words, \mintinline{Coq}{LFix} may  We could have
