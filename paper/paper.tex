...
schemes like mutual recursion, accumulators, primitive recursios, and
...
We now consider the formalisation hylomorphisms in Coq. We first formalise
...
section, we focus solely on quicksort.
...
%% Commenting out footnote to save space, since we mention the Coq QSort in related work already...
...
\end{coqcode}
While the extracted code has the expected performance characteristics, Coq's
code extraction mechanism is unable to inline several definitions in this case.
We have manually inlined the extracted code for simplicity. The full extracted
code is available in the artefact accompanying this paper, and an interested
reader can check that simple inlining would produce the following function:
