...
$i \times (1 + G_{\infty} A)$. This is the type representing our memory table
...
\end{coqcode}
Note the map \coq{App G Table ~> A} corresponding to a map $G G_{\infty}A \to A$ is not a $G$-algebra.
To recover the $G$-algebra we pair it with the identity map and  so we need to
transmogrify it by pairing it with the identity map and post-composing it with
\coq{consT}. This algebra uses this table to lookup
elements, instead of triggering a further recursive call, then elements are inserted
into the memoisation table by the use of \coq{consT} to the result of applying
the algebra. In our example this algebra is called \coq{knapsackA} (see accompanying code).
% \begin{coqcode}
...
% \end{coqcode}
Finally, given the recursive coalgebra \coq{out_nat} on the natural numbers, we can define knapsack using the recursion scheme for dynamorphisms:
\begin{coqcode}
Example knapsack wvs : Ext (dyna (knapsackA wvs) out_nat).
\end{coqcode}
The full extracted code coming from this specification
is available in the artefact accompanying this paper, and an interested reader
can check that simple inlining would produce the following:
...
\end{ocamlcode}
Note that in the extracted code, the recursive calls to \ocaml{f} build the memoisation table, and that
...
Instead, we prove the acid rain theorem of the specific parametric function
\coq{tau}. This essentially is a recursive function on lists which uses the
algebra for both the recursive step and for the base case where it is used in
the hylomorphism to continue the recursion:
\begin{coqcode}
