...
\begin{minted}{OCaml}
  ...
  let rec quicksort xs = match divide xs with
    | None -> []
    | Some (x, (smaller, larger)) -> quicksort smaller @ (x::quicksort larger)
\end{minted}
While \ocaml{quicksort} is a well-defined mathematical function it cannot be
...
particular, \ocaml{quicksort} is an instance of a \emph{divide-and-conquer}
...
parallelisations~\cite{Gibbons96:Third}.
...
example our mechanisation of \ocaml{quicksort} as a hylomorphism will produce
the following code:
\begin{coqcode}
...
Here we will call \emph{container functor} the functors defined
by a specific container extension.
...
Here consider instead representing the positions of a
container using a validity predicate which assigns shapes to positions.
...
Additionaly we would like to reason about equality of container extensions. The
contents of the container extensions need to be accessed via a
function \coq{contents} whose domain depends on the shape of the container.
...
\section{Certified Hylomorphisms}
