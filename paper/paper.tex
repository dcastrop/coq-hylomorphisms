...
actually an isomorphim with its inverse
\begin{minted}{coq}
In : unit + A * List A -> List A
\end{minted}
...
type
\begin{minted}{coq}
unit + A * List A -> unit + A * List B
\end{minted}
Once the recursion as returned it result it applies the algebra \coq{alg} which
composes together the results of the recursion yielding the result of
type \coq{B}.

More abstractly, assuming a base functor \coq{F} a catamorphism is the unique
