...
or introducing unsafe casts into extracted code (for example, \ocaml{Obj.magic}
...
% while enabling
% formal reasoning about program equivalences using the laws of hylomorphisms.
...
uses of \ocaml{Obj.magic}. We also present a machine-checked proof of the
well-known short-cut fusion optimsation.
...
\label{sec:algebras} An algebra is a set \coq{X} together with some operations
...
algebras}.  For example, give such a type \coq{X} we can define the monoid
operations as \coq{u : unit -> X} for unit and \coq{m : X * X -> X} for
multiplication. Notice that \coq{FX = unit + X * X} is a functor on \coq{X}
which means we can equivalently describe these two maps as a single one of
type \coq{F X -> X} which we call an algebra for the functor \coq{F}
...
\end{coqcode}
It is easy to show that this function is a morphism of \coq{F}-algebras. In fact,
...
carrier \coq{X} is a map \coq{X -> FX}. In the previous example \coq{c} is
an \coq{F}-coalgebra on the carrier \coq{X} with \coq{FX = L * X}.
...
unique \coq{F}-coalgebra homomorphism, that is a map of the following type
\begin{coqcode}
Coalg F A ~> A ~> GFix
\end{coqcode}
...
\end{coqcode}
We again use \coq{RecF_inv} to obtain the structurally smaller proof to use in
the recursive calls. As we did with catamorphisms and recursive anamorphisms, we
prove that \coq{hylo_def} is respectful, and use this proof to built the
...
Anamorphisms specialise to an hylomorphism into an inductive data type.
Intuitively, this is because a recursive coalgebra can be applied only finitely
many times, so when this is applied to an anamorphism the only possible traces
it can produce are the finite ones.  In particular, we can show that
anamorphisms of type \coq{X -> GFix F} applied to a recursive \coq{F}-coalgebra
on \coq{X} are in one-to-one correspondence with hylomorphisms of
type \coq{X -> LFix F} on the same recursive \coq{F}-coalgebra and the
algebra \coq{In} for the inductive data type \coq{LFix F}.

To do this we first define a predicate on elements of coinductive data
structures which holds if and only if the element is finite:
\begin{coqcode}
Inductive FinF : GFix F -> Prop :=
| FinF_fold (x : GFix F) : (forall e, FinF (cont (g_out x) e)) -> FinF x.
\end{coqcode}
We can now define catamorphisms denoted by \coq{ccata_f_} from the
...
\end{coqcode}
In particular, we can take a catamorphism from the finitary elements
of \coq{GFix F} to the inductive data type \coq{LFix F} using
the \coq{F}-algebra \coq{l_in} and viceversa using catamorphism on the
restriction of \coq{g_in} to the finite elements of \coq{GFix}, which we denote
by \coq{lg_in}.
\begin{coqcode}
...
\end{coqcode}
Moreover, using this new initial algebra, we can compose catamorphisms and
...
by \emph{using Coq's proofs of hylomorphism laws as optimisation passes}.
