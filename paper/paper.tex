...
\end{minted}
 ...
 As an example, we the quicksort program can be written as an hylomorphism
...
\noindent
For simplicity, we will abuse terminology, and we calling these
...
described as the least fixed point of \coq{AppR}. We define smart
...
\begin{coqcode}
Definition cata_f F (alg : Alg F A) : LFix -> A
  := fix f (x : LFix)
     := match x with
        | LFix_in ax =>
...
following type:
\begin{coqcode}
  cata : forall F `{setoid A}, Alg F A ~> LFix ~> A
\end{coqcode}
...
Definition hylo_def (a : Alg F B) (c : Coalg F A)
  ...
  := fix f x H
     := match c x as cx
        ...
        with
        | MkApp sx kx => fun H => a (MkApp sx (fun e => f (kx e) (H e)))
        end (RecF_inv H).
\end{coqcode}
...
For convenience, we package coalgebras with their proofs that they terminate for
all inputs:
\begin{coqcode}
