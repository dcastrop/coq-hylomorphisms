...
least ``sets'' generated by some constructors. For example, the type of lists
over a type \coq{A}, namely \coq{List A} is the ``least set'' generated by the
constructors \coq{Empty : unit -> List A} and \coq{Cons : A * List A -> List A}.
Together, \coq{Empty} and \coq{Cons} form an \emph{algebra} of
type \coq{In : unit + A * List A -> List A}. In particular, this algebra is an
isomorphism with the obvious inverse \coq{InOp : List A -> unit + A * List A}
returning \coq{tt} is the list is empty and the head and tail of the list
otherwise. This type of algebra is called \emph{initial algebra}.  This is
because for for any other algebra \coq{alg : unit + A * List B -> B} there
exists a unique map which we call \coq{cata alg : List A -> B} which destructs a
list into its head and tail using a
destructor \coq{InOp : List A -> unit + A * List A}, applies itself recursively,
composes together the results of the recursion using the algebra \coq{alg}.
...
Given a type `\coq{A}` and a container \coq{F} an algebra for \coq{F} over the
carrier `\coq{A}` is a map of the following type:
\begin{coqcode}
Notation Alg F A := (App F A ~> A).
\end{coqcode}
When \coq{A} is the least fixed-point of \coq{F} this map is an isomorphism and
it is called the \emph{initial algebra} and denotes the inductive data type shaped by the functor represented by \coq{F}:
\begin{coqcode}
