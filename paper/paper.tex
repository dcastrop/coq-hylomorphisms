...
\end{coqcode}
Note the map \coq{App G Table ~> A} corresponding to a map $G G_{\infty}A \to A$
is not a $G$-algebra.  To recover the $G$-algebra we pair it with the identity
map and  so we need to transmogrify it by pairing it with the identity map and
