...
polynomial functors. To ensure that the extracted code is runnable, avoid using
indexed types in the definition of our recursion schemes. To strengthen the
results of this paper, we avoid all axioms.
...
s_2$. In Coq, if we know $(s_1 , f) = (s_2 , g)$, we cannot extract a proof that
$f = g$ unless we assume axiom K, or the equality on the type of $s_1$ and $s_2$
is decidable.  Furthermore, upon extraction Coq will need to insert unsafe casts
for
...
define it as a setoid morphism between the setoid of \coq{F}-algebras and the
...
like this cannot be directly accepted by Coq.
