...
isomorphic to the type \coq{unit + A * X}. The correspondence between containers
and polynomial functors has been formalised in this work and has been moved to
the Appendix~\ref{sec:pol-containers} for space reasons.

Generally, any functor that is isomorphic to a container extension is called a
\emph{container functor}. However, in our work we will call \emph{container
functor} the functors defined by a specific container extension.
...
possible shapes, together with a predicate \coq{Valid} that determines whether a
