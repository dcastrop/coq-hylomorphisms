...
Definition Is := unit.
Definition Ip := unit.
Instance Id : Container Is Ip.
refine {| dom := {| app := fun _ => true|} |}.
intros. reflexivity.
Defined.
...
Definition Kp := False.
Instance K A : Container (Ks A) Kp.
refine {| dom := {| app := fun x : Ks A * Kp  => match snd x with end |} |}.
...
Section Product.
  ...
  Context (C1 : Container S1 P1) (C2 : Container S2 P2).
  Notation Ps := ((S1 * S2)%type).
  Notation Pp := ((P1 + P2)%type).
  Instance Prod : Container Ps Pp.
  refine {| dom := {| app := fun x : Ps * Pp  =>
                               match snd x with
                               | inl p => dom (fst (fst x), p)
                               | inr p => dom (snd (fst x), p)
                               end |} |}.
...
Section Composition.
  Context `(F : Container S1 P1) `(G : Container S2 P2).

  Definition composeDom :
  ...
  #[export]
    Instance CompCont : Container (App F S2) (P1 * P2) := { dom := composeDom }.
End Composition.
